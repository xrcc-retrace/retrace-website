...
function DitheredWaves({
  ...
  // Global pointer listener so Dither reacts to mouse even when canvas is behind DOM content
  useEffect(() => {
  ...
  useFrame(({ clock }) => {
    // Use the actual material uniforms via ref — bypasses R3F v9 reconciler
    const u = matRef.current?.uniforms ?? waveUniformsRef.current;
